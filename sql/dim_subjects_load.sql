...
WITH CTE AS (
	...
	FROM "STAGE_PASSEI_DIRETO".stg_dim_subjects
	ORDER BY id
    ),
-----------------
---- Slow Change Dimession ----
    UPD AS (
        UPDATE "DM_PASSEI_DIRETO".dim_subjects T
...
--------------------------------
---- Target ----
    INSERT INTO "DM_PASSEI_DIRETO".dim_subjects(id, "name", change_date )
    SELECT id, "name", NULL FROM CTE
    WHERE CTE.id IN (SELECT id FROM UPD) OR
        CTE.id NOT IN (SELECT id FROM "DM_PASSEI_DIRETO".dim_subjects)
----------------